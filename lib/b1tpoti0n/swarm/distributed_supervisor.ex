defmodule B1tpoti0n.Swarm.DistributedSupervisor do
  @moduledoc """
  Distributed dynamic supervisor for swarm workers using Horde.

  When clustering is enabled, uses Horde.DynamicSupervisor for distributed
  process supervision. Workers are automatically distributed across nodes.
  """
  ...
  alias B1tpoti0n.Swarm.DistributedRegistry
  ...
  Uses Horde.Registry for O(1) lookup across the cluster.
          ...
          {:ok, torrent} ->
            case start_worker(info_hash, torrent.id) do
              ...
              {:error, {:already_started, pid}} ->
                # Race condition: another node started the worker
                {:ok, pid}
  ...
  @doc """
  Look up an existing swarm worker by info_hash across the cluster.
  """
  @spec lookup_worker(binary()) :: {:ok, pid()} | :error
  def lookup_worker(info_hash) do
    case Horde.Registry.lookup(DistributedRegistry, info_hash) do
      [{pid, _value}] -> {:ok, pid}
      [] -> :error
    end
  end
  ...
  def start_worker(info_hash, torrent_id) do
    spec = {Worker, {info_hash, torrent_id, :distributed}}
  ...
  @doc """
  Get the count of active swarm workers across the cluster.
  """
  @spec count_workers() :: non_neg_integer()
  def count_workers do
    Horde.DynamicSupervisor.count_children(__MODULE__)[:active]
  end

  @doc """
  List all active info_hashes across the cluster.
  """
  @spec list_torrents() :: [binary()]
  def list_torrents do
    Horde.Registry.select(DistributedRegistry, [{{:"$1", :_, :_}, [], [:"$1"]}])
  end

  @doc """
  List all active swarm workers with their info_hashes across the cluster.
  Returns a list of {info_hash, pid} tuples.
  """
  @spec list_workers() :: [{binary(), pid()}]
  def list_workers do
    Horde.Registry.select(DistributedRegistry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
  end
end
